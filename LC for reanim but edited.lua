...
local PositionOffset = Vector3.new(2.025, 1.87, 0)
local RotationOffset = Vector3.new(180,0, 135)
RotationOffset = CFrame.Angles(
   math.rad(RotationOffset.X),
   math.rad(RotationOffset.Y),
   math.rad(RotationOffset.Z)
)
local CFrameOffset = CFrame.new(PositionOffset)*RotationOffset

if eee:FindFirstChild("Accessory (VoidRPG)") then 
    eee["Accessory (VoidRPG)"].Handle:BreakJoints()
    weld3.C1 = CFrameOffset
    weld3.Parent = eee["Accessory (VoidRPG)"].Handle
    weld3.Part0 = eee["Accessory (VoidRPG)"].Handle
    weld3.Part1 = GUN.Base
end

local PositionOffset2 = Vector3.new(-1.85, -1.687, 0)
local RotationOffset2 = Vector3.new(180,0, 135)
RotationOffset2 = CFrame.Angles(
   math.rad(RotationOffset2.X),
   math.rad(RotationOffset2.Y),
   math.rad(RotationOffset2.Z)
)
local CFrameOffset2 = CFrame.new(PositionOffset2)*RotationOffset2

if eee:FindFirstChild("Accessory (VoidAlligatorJaw)") then 
    eee["Accessory (VoidAlligatorJaw)"].Handle:BreakJoints()
    weld2.C1 = CFrameOffset2
    weld2.Parent = eee["Accessory (VoidAlligatorJaw)"].Handle
    weld2.Part0 = eee["Accessory (VoidAlligatorJaw)"].Handle
    weld2.Part1 = GUN.Base
end

if eee:FindFirstChild("Accessory (AlligatorJaw)") then 
    eee["Accessory (AlligatorJaw)"].Handle:BreakJoints()
    weld2.C1 = CFrameOffset2
    weld2.Parent = eee["Accessory (AlligatorJaw)"].Handle
    weld2.Part0 = eee["Accessory (AlligatorJaw)"].Handle
    weld2.Part1 = GUN.Base
end

if eee:FindFirstChild("Accessory (RPG)") then 
    eee["Accessory (RPG)"].Handle:BreakJoints()
    weld3.C1 = CFrameOffset
    weld3.Parent = eee["Accessory (RPG)"].Handle
    weld3.Part0 = eee["Accessory (RPG)"].Handle
    weld3.Part1 = GUN.Base
end
-- here comes the shit code!
local asdf = workspace.DuskySpoon
local Hand1 = asdf["Accessory (Hnad)"]
local Hand2 = asdf["Accessory (Hand)"]
...
Hand1.Handle.AccessoryWeld:Destroy()
Hand1.Handle.CanCollide = false
Hand2.Handle.AccessoryWeld:Destroy()
Hand2.Handle.CanCollide = false
...
local Torso = asdf:FindFirstChild("Torso") or asdf:FindFirstChild("UpperTorso")
...
local BruhCollider = Instance.new("Part")
BruhCollider.Size = Vector3.new(1,1,1)
BruhCollider.Transparency = 1
BruhCollider.Anchored = false
BruhCollider.CanCollide = false
BruhCollider.CFrame = BruhEffect.CFrame
BruhCollider.Parent = workspace

AlignCharacter(BruhEffect, BruhCollider, Vector3.new(0, 0, 0), Vector3.new(0, 0, 0))

local BruhWeld = Instance.new("Weld")
BruhWeld.Part0 = Torso
BruhWeld.Part1 = BruhCollider
BruhWeld.C0 = CFrame.new(0, 4, 0)
BruhWeld.Parent = BruhCollider

local spin = Instance.new("BodyAngularVelocity")
spin.Parent = BruhEffect
spin.MaxTorque = Vector3.new(0, math.huge, 0)
spin.AngularVelocity = Vector3.new(0, 50, 0)

local TweenService = game:GetService("TweenService")
local going = false
local bruh = Enum.EasingStyle.Bounce 
local brbr = Enum.EasingDirection.Out 

local HANDCOLL = Instance.new("Part")
HANDCOLL.Size = Vector3.new(1,1,1)
HANDCOLL.Transparency = 1
HANDCOLL.Anchored = false
HANDCOLL.CanCollide = false
HANDCOLL.CFrame = Hand1.Handle.CFrame
HANDCOLL.Parent = workspace

AlignCharacter(Hand1.Handle, HANDCOLL, Vector3.new(0,0,0), Vector3.new(-80,0,0))

local HandWeld = Instance.new("Weld")
HandWeld.Part0 = Torso
HandWeld.Part1 = HANDCOLL
HandWeld.C0 = CFrame.new(0, -2.2, 0)
HandWeld.Parent = HANDCOLL


local HAND2COLL = Instance.new("Part")
HAND2COLL.Size = Vector3.new(1,1,1)
HAND2COLL.Transparency = 1
HAND2COLL.Anchored = false
HAND2COLL.CanCollide = false
HAND2COLL.CFrame = Hand2.Handle.CFrame
HAND2COLL.Parent = workspace

AlignCharacter(Hand2.Handle, HAND2COLL, Vector3.new(0,0,0), Vector3.new(80,0,0))

local Hand2Weld = Instance.new("Weld")
Hand2Weld.Part0 = Torso
Hand2Weld.Part1 = HAND2COLL
Hand2Weld.C0 = CFrame.new(0, 4, 0)
Hand2Weld.Parent = HAND2COLL

local TweenService = game:GetService("TweenService")

spawn(function()
    while true do
        local tweenUp = TweenService:Create(Hand2Weld, TweenInfo.new(3, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {C0 = CFrame.new(0, 5, 0)})
        tweenUp:Play()
        tweenUp.Completed:Wait()
        local tweenDown = TweenService:Create(Hand2Weld, TweenInfo.new(3, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {C0 = CFrame.new(0, 4, 0)})
        tweenDown:Play()
...
spawn(function()
    while true do
        local tweenDown = TweenService:Create(HandWeld, TweenInfo.new(3, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {C0 = CFrame.new(0, -4, 0)})
        tweenDown:Play()
        tweenDown.Completed:Wait()
        local tweenUp = TweenService:Create(HandWeld, TweenInfo.new(3, Enum.EasingStyle.Quad, Enum.EasingDirection.InOut), {C0 = CFrame.new(0, -2.2, 0)})
        tweenUp:Play()
        tweenUp.Completed:Wait()
    end
end)
...
spawn(function()
    while true do
        local targetUp = Torso.CFrame * CFrame.new(0, -2.5, 0) * CFrame.Angles(math.rad(-80),0,0)
        local tweenUp = TweenService:Create(HandCollider, TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {CFrame = targetUp})
        tweenUp:Play()
        tweenUp.Completed:Wait()

        local targetDown = Torso.CFrame * CFrame.new(0, -3.5, 0) * CFrame.Angles(math.rad(-80),0,0)
        local tweenDown = TweenService:Create(HandCollider, TweenInfo.new(0.3, Enum.EasingStyle.Sine, Enum.EasingDirection.Out), {CFrame = targetDown})
        tweenDown:Play()
        tweenDown.Completed:Wait()
    end
end)
...
Mouse.Button1Down:Connect(function()
    ...
    local tween = TweenService:Create(FakeCollider, TweenInfo.new(0.05, bruh, brbr), {CFrame = CFrame.new(targetPos)}) 
    tween:Play()
    tween.Completed:Wait()
    local tweenBack = TweenService:Create(FakeCollider, TweenInfo.new(0.07, bruh, brbr), {CFrame = Torso.CFrame * CFrame.new(0, 5, 0)}) 
    tweenBack:Play()
...
game:GetService("RunService").Heartbeat:Connect(function()
    bambam.Force = Bullett.AssemblyLinearVelocity * 25.1
end)

spawn(function()
    while true do
        spin.AngularVelocity = Vector3.new(0, 50, 0)
        local fastTime = math.random(1,2)/2
        wait(fastTime)
        spin.AngularVelocity = Vector3.new(0, 20, 0)
        local normalTime = math.random(2,3)
        wait(normalTime)
    end
end)
wait(0.1)
BruhCollider.AliO_2.MaxTorque = 0
